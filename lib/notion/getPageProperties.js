...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  console.log('调试POST_URL_PREFIX_MAPPING_CATEGORY - POST_URL_PREFIX_MAPPING_CATEGORY:', POST_URL_PREFIX_MAPPING_CATEGORY) 
   
  /**
  const POST_URL_PREFIX_MAPPING_CATEGORY = {
    "知行合一":"learning",
    "技术分享":"technology",
    "心情随笔":"essay"
  }
  */
  const POST_URL_PREFIX_MAPPING_CATEGORY2 = process.env.POST_URL_PREFIX_MAPPING_CATEGORY2 || {}
  console.log('调试POST_URL_PREFIX_MAPPING_CATEGORY2 - POST_URL_PREFIX_MAPPING_CATEGORY2:', POST_URL_PREFIX_MAPPING_CATEGORY2) 
  ...
  allSlugPatterns.forEach((pattern, idx) => {
    if (pattern === '%year%' && postProperties?.publishDay) {
      ...
      console.log('调试1 - 原始categoryPrefix:', categoryPrefix) // 添加log查看原始值
      // 允许映射分类名，通常用来将中文分类映射成英文，美化url.
      if (POST_URL_PREFIX_MAPPING_CATEGORY[postProperties?.category]) {
        console.log('调试2 - 找到映射，原始分类:', postProperties?.category, '映射目标:', POST_URL_PREFIX_MAPPING_CATEGORY[postProperties?.category]) // 添加log查看是否进入映射
        categoryPrefix =
          POST_URL_PREFIX_MAPPING_CATEGORY[postProperties?.category]
        console.log('调试3 - 映射后categoryPrefix:', categoryPrefix) // 添加log查看映射后的值
      }
      console.log('调试5 - 最终categoryPrefix:', categoryPrefix) // 添加log查看最终值
